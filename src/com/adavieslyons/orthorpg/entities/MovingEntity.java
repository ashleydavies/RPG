...
public abstract class MovingEntity extends Entity {
	...
	protected void updateMove(int delta) {
		if (moving) {
			...
			setRenderPosition(previousPosition.lerpTo(occupiedPosition, tileMoveCurrently));
			
			if (tileMoveCurrently >= 1) {
				tileMoveCurrently = 0;
				moving = false;
				setRenderPosition(new Vector2f(occupiedPosition.getX(), occupiedPosition.getY()));
			}
		}
